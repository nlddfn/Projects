...
####################
make_blend = function(model_list, threshold, type_list, comment = 'blend'){
  
  for(i in range(length(model_list))){
      # Calculate final prediction
      if(type_list[i] == 'xgb'){
        xgb_pred = predict(model_list[i], as.matrix(dfTest[,2:31]),missing=NA)}
      if(type_list[i] == 'ada'){ 
        ada_pred = predict(model_list[i], select(dfTest,c(-EventId)),type = 'probs')}
    }
  pred = (xgb_pred + ada_pred[,2])/2
